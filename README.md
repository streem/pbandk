...
* Support all platforms that are supported by Kotlin Multiplatform (JVM, Android, iOS, JS, WasmJs, WasmWasi, Linux, etc.)
...
* Works on JVM, Android, and Native (iOS, macOS, Linux, etc.), with experimental support for JS and Wasm
...
* Kotlin/JS support
* Kotlin/Wasm support
    * There is no support for using pbandk APIs and pbandk-generated code directly from Javascript code (or other Wasm host languages). They can only be called from other Kotlin code that is being compiled for Kotlin/Wasm. Kotlin/Wasm doesn't currently support using classes or complex types as arguments for Kotlin functions that are exposed to the Wasm host environment. This makes it infeasible to expose pbandk APIs directly outside of Kotlin.
